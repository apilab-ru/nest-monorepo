...
export class UsersService {
   ...
   async updateUserSongs(songs: string[], userId: number): Promise<UserSongsEntity[]> {
   ...
   async updateUserArtists(artists: string[], userId: number): Promise<UserArtistEntity[]> {
   ...
   async registration(params: UserRegParams): Promise<User> {
      ...
      return this.repositoryUser.findOneBy({
         email: params.email,
      }).then(user => {
         if (user) {
