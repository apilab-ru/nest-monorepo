...
export class SongsService {
  ...
  loadSongs(): Observable<any> {
    return from(
      ...
      switchMap((list) =>
        combineLatest(
          list.map((item) =>
            this.songsApiService.loadSongMeta(item.name, item.band),
          ),
        ).pipe(
          map((results) => {
          ...
          switchMap((results) => {
            ...
            const updatedSongsWithTags = results
              .filter((item) => item.newTags?.length)
              .map((song) => ({
                ...song,
                tags: [...song.tags, ...song.newTags],
              }));

            const skippedSongs = results.filter(
              (item) => !item.newTags?.length,
            );

            return combineLatest([
              this.songRepo.save(skippedSongs),
              ...updatedSongsWithTags.map((song) =>
                this.updateSongWithTag(song),
              ),
            ]).pipe(
              map(() => ({
  ...
  private updateSongWithTag(song: SongEntity): Observable<void> {
    return combineLatest([
      from(this.songRepo.save(song)).pipe(map(() => {})),
      from(
        ...
        map((maps) =>
          maps.map((item) => ({
            ...item,
            tags: [...item.tags, ...this.newTags(item.tags, song.tags)],
          })),
        ),
  ...
  parseSongsFromMaps(): Observable<void> {
    return from(
      ...
      switchMap((entities) => {
        ...
        return combineLatest([
          ...Object.values(recordBySong).map((items) =>
            from(this.addSong(items[0])).pipe(
              tap((song) =>
                items.forEach((item) => {
                  const mapItem = item.mapItem;

                  mapItem.songId = song.id;
                  mapItem.tags.push(...this.newTags(mapItem.tags, song.tags));
