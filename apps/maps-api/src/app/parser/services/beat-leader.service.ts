...
import { BeatLeaderItem, BeatLeaderResponse, BeatLeaderResponseItem } from "../interfaces/beat-leader";
...
export class BeatLeaderService {
  ...
  updateBeatLeaders(): Observable<any[]> {
    const lastDate$ = from(this.mapsService.loadList({
      blRanked: 'true',
    ...
    return lastDate$.pipe(
      ...
      switchMap(async list => {
        ...
        return result.map(item => ({ id: item.id, blRankedDate: item.blRankedDate }))
      })
    )
  }

  private loadRankedSongs(dateFrom?: number, page = 1): Observable<BeatLeaderItem[]> {
    if (isNaN(dateFrom)) {
      dateFrom = 0;
    }
  ...
  private loadPage(page: number, dateFrom?: number): Observable<BeatLeaderItem[]> {
    return this.httpService.get<BeatLeaderResponse>(this.api + 'leaderboards', {
      params: {
        page,
        count: this.limit,
        type: 'ranked',
  ...
  private mapResponseItem(item: BeatLeaderResponseItem): BeatLeaderItem {
    ...
    return {
      ...
      rankedDate: !item.difficulty.rankedTime
        ? this.uncertainDate
        : format(new Date(item.difficulty.rankedTime * 1000), 'yyyy-MM-dd hh:mm:ss'),
