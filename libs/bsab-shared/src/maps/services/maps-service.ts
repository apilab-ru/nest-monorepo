...
export class MapsService {
  ...
  constructor(connection: Connection, private errorsService: ErrorsService) {
  ...
  updateItem(item: MapEntity): Observable<MapEntity> {
    return from(this.repository.save(item)).pipe(map((result) => item));
  }
  ...
  async loadList(
    ...
    maxLimit = 100,
    userId?: number,
  ): Promise<PageResponse<MapEntity>> {
    ...
    if (query.tagsPositive) {
      ...
      if (ids.length) {
        queryRunner.andWhere(
          '(' +
            ids
              .map((tag) => `JSON_CONTAINS(maps.tags, "${tag}")`)
              .join(' AND ') +
            ')',
        );
      }
    ...
    if (query.tagsNegative) {
      ...
      if (ids.length) {
        queryRunner.andWhere(
          '(' +
            ids
              .map((tag) => `not JSON_CONTAINS(maps.tags, "${tag}")`)
              .join(' AND ') +
            ')',
        );
      }
    ...
    if (query.showed !== undefined && userId) {
      ...
      queryRunner.andWhere(
        `userShown.userId ${
          query.showed === 'true' ? '= ' + userId : 'IS NULL'
        }`,
        {
    ...
    const orderDirection =
      OrderDirectionDTO[query.orderDirection] || OrderDirectionDTO.desc;
